...
@dataclass
class VENIEquirectangularPixelSamplerConfig(PixelSamplerConfig):
    """Configuration for pixel sampler instantiation."""

    _target: Type = field(default_factory=lambda: VENIEquirectangularPixelSampler)
...
class VENIEquirectangularPixelSampler(PixelSampler):
    ...
    def __init__(
        self,
        config: VENIEquirectangularPixelSamplerConfig,
        **kwargs,
    ):
    ...
    def collate_image_dataset_batch(
        ...
        used in VENI as a gradient based loss where the gradient is approximated using finite-difference.
